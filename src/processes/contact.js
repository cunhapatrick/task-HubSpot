const { filterNullValuesFromObject, saveDomain, generateLastModifiedDateFilter, refreshAccessToken } = require('../utils');

/**
 * Processes contacts from HubSpot and updates the domain with the latest contact information.
 ...
 * @param {Object} queue - The queue to push contact actions.
 ...
 *
 * @throws {Error} Throws an error if contacts cannot be fetched after multiple attempts.
 */
const processContacts = async (domain, hubId, queue, expirationDate, hubspotClient) => {
  ...
  companyAssociationsResults
    .filter(companyAssociation => companyAssociation?.from && companyAssociation?.to?.length > 0)
    .map(companyAssociation => {
      const contactId = companyAssociation.from.id;
      const companyId = companyAssociation.to[0].id;
